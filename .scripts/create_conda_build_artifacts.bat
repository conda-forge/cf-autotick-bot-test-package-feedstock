...
if defined BLD_ARTIFACT_PREFIX (
    ...
    tar -c -v -f - "%ZSTD%" %EXCLUDE_FROM_BUILD_ARTIFACTS% . >"!BLD_ARTIFACT_PATH!"
    if errorlevel 1 exit 1
...
if defined WRK_ARTIFACT_PREFIX (
    ...
    tar -c -v -f - "%ZSTD%" %EXCLUDE_FROM_WORK% %WORK_PATHS% >"!WRK_ARTIFACT_PATH!"
    if errorlevel 1 exit 1
...
if defined ENV_ARTIFACT_PREFIX (
    ...
    tar -c -v -f - "%ZSTD%" %ENVIRONMENT_PATHS% >"!ENV_ARTIFACT_PATH!"
    if errorlevel 1 exit 1
